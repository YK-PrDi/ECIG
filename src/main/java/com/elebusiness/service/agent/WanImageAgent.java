...
@Service
public class WanImageAgent implements ImageGeneratorAgent {

    private static final Logger log = LoggerFactory.getLogger(WanImageAgent.class);
    private static final String CREATE_URL = "https://dashscope.aliyuncs.com/api/v1/services/aigc/image-generation/generation";
    ...
    public WanImageAgent(DashScopeConfig config) {
        this.config = config;
    }

    @Override
    public String getId() { return "wan2.7"; }
    ...
    public boolean generate(String prompt, String refImagePath, String whiteBgPath, String outputPath) {
        ...
        try {
            ...
            log.info("万相图片已保存: {}", outputPath);
            return true;
        } catch (Exception e) {
            log.error("万相生成失败: {}", e.getMessage());
            return false;
        }
    }
    ...
    public String generateUrl(String prompt) {
        ...
        try {
            ...
            throw new RuntimeException("万相调用失败: " + e.getMessage(), e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("万相任务被中断");
        }
    }
    ...
    private String pollTask(OkHttpClient client, String taskId) throws IOException, InterruptedException {
        ...
        for (int i = 0; i < 90; i++) {
            // 前 5 次每 2 秒轮询一次，之后每 3 秒一次
            Thread.sleep(i < 5 ? 2000 : 3000);
            try (Response response = client.newCall(request).execute()) {
                String responseBody = response.body().string();
                JsonNode root = objectMapper.readTree(responseBody);
                String status = root.path("output").path("task_status").asText();
                log.info("万相轮询第 {} 次，状态: {}", i + 1, status);

                if ("SUCCEEDED".equals(status)) return extractImageUrl(root, responseBody);
                if ("FAILED".equals(status)) throw new RuntimeException("万相任务失败: " + responseBody);
            }
        }
        throw new RuntimeException("万相任务超时（180秒内未完成）");
    }
